...
void increase_troops(struct Regions region[],int region_size,struct Players player[],int flag) {
    ...
    int is_in_player_center(struct Regions region[], int region_size, int x, int y, struct Players player[]) {
        for (int i = 0; i < region_size; i++) {
            if (x > region[i].center_x * 10 - 20 && x < region[i].center_x * 10 + 10 &&
                y > region[i].center_y * 10 - 20 && y < region[i].center_y * 10 + 10 && region[i].id == 0) {
                return i;
            }
        }
        return -1;
    ...
    int is_in_center(struct Regions region[], int region_size, int x, int y) {
        for (int i = 0; i < region_size; i++) {
            if (x > region[i].center_x * 10 - 20 && x < region[i].center_x * 10 + 10 &&
                y > region[i].center_y * 10 - 20 && y < region[i].center_y * 10 + 10) {
                return i;
            }
        }
        return -1;
    ...
    int map(SDL_Renderer *sdlRenderer, int game_map[][map_x], int region_size, int players, Uint32 colors_array[],
        ...
        draw_map(region, sdlRenderer, region_size, colors_array, players, barracks_colors_array, 1, player);
                ...
                if (squad[i].is_empty == 0) {
                    ...
                    for (int j = 0; j < squad[i].counter; j++) {
                        ...
                        if (squad[i].x[j] != -1) {
                            ...
                            if(j%3==0) {
                                squad[i].Vx = Vo * ((region[squad[i].destination_region_index].center_x * 10-5 - squad[i].x[j]) /
                                                    (sqrt(pow((region[squad[i].destination_region_index].center_x * 10 -5 - squad[i].x[j]), 2) +
                                                          pow((region[squad[i].destination_region_index].center_y * 10 - 5 - squad[i].y[j]), 2))));
                                squad[i].Vy = Vo * ((region[squad[i].destination_region_index].center_y * 10 -5 - squad[i].y[j]) /
                                                    (sqrt(pow((region[squad[i].destination_region_index].center_x * 10 -5 - squad[i].x[j]), 2) +
                                                          pow((region[squad[i].destination_region_index].center_y * 10 -5 - squad[i].y[j]), 2))));
                            }
                        ...
                        if (squad[i].x[j] <= region[squad[i].destination_region_index].center_x * 10+20 &&
                            squad[i].x[j] >= region[squad[i].destination_region_index].center_x * 10 - 30 &&
                            squad[i].y[j] <= region[squad[i].destination_region_index].center_y * 10+20 &&
